...
\term{GAGA}
Serre's comparison principle asserting that for complex projective varieties the coherent analytic sheaves and their cohomology coincide with their algebraic counterparts; the Riemann existence theorem arises as a special case. The theorem bridges algebraic and analytic geometry, showing that the categories of algebraic and analytic coherent sheaves are equivalent and that the corresponding algebraic and analytic cohomology spaces are canonically isomorphic.
...
A highly efficient numerical integration method that selects optimal sample points (nodes) and weights to exactly integrate polynomials of a certain degree. For example, Gauss--Legendre quadrature uses roots of Legendre polynomials to provide the best possible accuracy for a given number of points over a finite interval.
...
\term{Gelfand Triple}
A Gelfand triple, or rigged Hilbert space, is a chain $\Phi \subset H \subset \Phi^*$ in which $\Phi$ is a nuclear space densely and continuously embedded in a Hilbert space $H$, and $\Phi^*$ is the dual of $\Phi$ containing $H$. It provides the functional-analytic setting for the Schwartz distributions and, via the nuclear spectral theorem, for generalized eigenfunctions of self-adjoint operators, enabling a spectral decomposition where eigenvalues may lie in the continuous spectrum.
...
See Cut Vertex. An articulation point or cut vertex is a vertex whose removal increases the number of connected components. The term is standard in graph theory and network analysis.
\term{Canonical Transformation}
A change of coordinates in phase space that preserves the form of Hamilton's equations. In symplectic geometry, a canonical transformation is a symplectomorphism: a diffeomorphism $\phi$ such that $\phi^*\omega = \omega$, where $\omega = \sum dq_i \wedge dp_i$ is the standard symplectic form. Generating functions provide concrete realisations.
