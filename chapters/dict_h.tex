...
In $\mathbb{R}^n$, a subset is compact if and only if it is closed and bounded. This characterisation links a topological property (compactness) to metric properties (closedness and boundedness) and is essential for proving the existence of extrema for continuous functions.
...
A functor $F: \mathcal{C} \to \mathcal{D}$ that reverses the direction of morphisms: $F(g \circ f) = F(g) \circ F(f)$. Equivalently, a covariant functor from $\mathcal{C}^{\operatorname{op}}$ to $\mathcal{D}$. The contravariant Hom functor $\operatorname{Hom}(-, A)$ and the dual space functor $V \mapsto V^*$ are fundamental examples. Contravariant functors appear throughout algebra and topology, particularly in cohomology theory.
