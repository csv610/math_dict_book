...
A way of assigning a real number to every 2-plane in the tangent space of a Riemannian manifold, representing the Gaussian curvature of the surface formed by the geodesics passing through the point and tangent to the plane.
...
\term{Schwarz Space}
See \term{Schwartz Space}.

\term{Schwartz Space}
The Schwartz space $\mathcal{S}(\mathbb{R}^n)$ consists of smooth functions whose derivatives decrease faster than every power of $1/\lVert x\rVert$ as $\lVert x\rVert\to\infty$. The Fourier transform maps this space to itself. Applications: it is the natural setting for Fourier analysis, signal processing, partial differential equations, and distributions.
...
A group $G$ is a semidirect product $N \rtimes H$ when $N$ is normal, $H$ is a subgroup, $G = NH$, and $N \cap H = \{e\}$; $H$ acts on $N$ by automorphisms and multiplication is given by $(n_1,h_1)(n_2,h_2) = (n_1 h_1 \cdot n_2, h_1 h_2)$. The group then splits over $N$. Semidirect products classify groups with a prescribed normal factor: dihedral groups $D_{2n}$, the symmetric group $S_n = \mathbb{Z}/2\mathbb{Z} \ltimes A_n$, and the affine group of $\mathbb{R}^n$ are standard examples.
...
A set of axioms (such as $T_0, T_1, T_2, T_3, T_4$) that describe the ability to distinguish distinct points or closed sets in a topological space using open sets.
...
The cohomology groups $H^i(X, \mathcal{F})$ of a sheaf $\mathcal{F}$ on a topological space or scheme $X$, computed as the right derived functors of the global sections functor. They may be computed via Čech cohomology or by acyclic resolutions such as flasque, injective, or Godement resolutions; these definitions agree on paracompact spaces. The groups govern the existence of global sections, with $H^1(X, \mathcal{F})$ classifying $\mathcal{F}$-torsors and $H^2$ classifying gerbes bound by $\mathcal{F}$, making them central to obstruction theory and descent.
...
\term{Signed Measure}
A countably additive set function on a measurable space taking values in $\mathbb{R} \cup \{-\infty, +\infty\}$, with at most one of the two infinite values. By the Hahn decomposition the space splits into a positive and a negative set for $\nu$; the Jordan decomposition then writes $\nu = \nu^+ - \nu^-$ as the difference of mutually singular nonnegative measures. The total variation $|\nu| = \nu^+ + \nu^-$ is itself a measure. Signed measures arise in the Radon--Nikodym theorem and in duality for $L^p$ spaces.
...
\term{Simplicial Set}
A functor $X: \Delta^{\mathrm{op}} \to \mathbf{Set}$ from the opposite of the simplex category $\Delta$ to the category of sets, equipped with face and degeneracy maps satisfying the simplicial identities. Simplicial sets form a purely combinatorial model for homotopy theory: the fibrant objects, Kan complexes, satisfy the extension condition and their geometric realizations $|X|$ encode homotopy types. The category of simplicial sets carries a Quillen model structure whose weak equivalences are maps inducing weak equivalences on realizations, making it a model for the homotopy category of spaces.

\term{Simpson's Paradox}
A trend that holds in each separate subgroup of data can be reversed when the subgroups are pooled together. It arises from lurking or confounding variables and unequal group sizes; the classic example is a treatment that looks better for every patient group yet worse overall.

\term{Singular Integral}
An operator of the form $Tf(x) = \operatorname{p.v.}\int_{\mathbb{R}^n} K(x-y) f(y) \, dy$, where the kernel $K$ is homogeneous of degree $-n$ and has zero mean on the unit sphere; the principal value handles the singularity at $y = x$. The Hilbert transform on $\mathbb{R}$ and the Riesz transforms on $\mathbb{R}^n$ are the basic examples. By the Calderón--Zygmund theory such operators are bounded on $L^p$ for $1 < p < \infty$, making them central to elliptic PDE and harmonic analysis.
...
A variety that contains at least one point where the dimension of the tangent space is strictly greater than the dimension of the variety. The set of all such points is the singular locus, and its complement is the smooth locus.
...
A path-connected topological space $X$ is simply connected if its fundamental group is trivial, meaning every loop in $X$ can be continuously contracted to a point. Examples include the $n$-sphere $S^n$ for $n \geq 2$ and the Euclidean space $\mathbb{R}^n$.

\term{Smooth Manifold}
A topological manifold equipped with a smooth atlas of charts whose transition maps are infinitely differentiable.

\term{Smooth Variety}
A variety $X$ of dimension $n$ such that the tangent space at every point $p \in X$ has dimension exactly $n$. Smooth varieties are manifolds over the base field $k$ and are the central objects of classical algebraic geometry.
...
\term{Soluble Group}
A group possessing a finite subnormal series $1 = G_0 \triangleleft G_1 \triangleleft \cdots \triangleleft G_n = G$ with each quotient $G_i/G_{i-1}$ abelian; equivalently, the derived series terminates at the trivial subgroup. By the Feit--Thompson theorem every group of odd order is solvable. Soluble groups are precisely those finite groups whose composition factors are cyclic of prime order, connect to radicals and to the solvability of polynomials by radicals, and are closely related to nilpotent groups.

\term{Solvable by Radicals}
A polynomial equation $f(x) = 0$ over $\mathbb{Q}$ is solvable by radicals when its roots can be expressed from rational numbers using only $+$, $-$, $\times$, $\div$, and extraction of $n$-th roots. Galois's theorem characterizes this: $f$ is solvable by radicals if and only if its Galois group is a soluble group. Consequently the general quintic is not solvable by radicals, since $S_5$ is not soluble, resolving the classical problem of solving equations by radicals.
...
A bookkeeping device in homological algebra that computes (co)homology through successive approximations. Starting from an initial page such as $E_0$ or $E_2$, each page $E_r$ is a complex with differential $d_r$, and $E_{r+1}$ is the homology of $E_r$; under mild conditions the pages converge to a limit $E_\infty$, the associated graded of the target, called the abutment. Examples include the Grothendieck spectral sequence for composite functors, the Leray--Serre sequence of a fibration, the Atiyah--Hirzebruch sequence in topological K-theory, and the Adams spectral sequence in stable homotopy.
...
For a bounded linear operator $T$ on a Banach space, the set of scalars $\lambda$ such that $T - \lambda I$ is not invertible. It generalizes the set of eigenvalues of a matrix to infinite-dimensional spaces.
...
\term{Splitting Field}
The smallest field extension of $K$ over which a given polynomial $f$ factors into linear factors; equivalently, the extension generated by all the roots of $f$ over $K$. It always exists and is unique up to isomorphism over $K$. Every splitting field is a normal, and hence a Galois, extension when it is separable; the automorphism group of the splitting field of $f$ is its Galois group. The splitting field of $x^n - 1$ is the cyclotomic extension with Galois group $(\mathbb{Z}/n\mathbb{Z})^\times$.
...
A sequence of pointed spaces $\{X_n\}$ together with structure maps $S^1 \wedge X_n \to X_{n+1}$, where $\wedge$ denotes the smash product. Spectra form the stable homotopy category, obtained by inverting the suspension functor, and represent generalized (co)homology theories: the sphere spectrum represents stable homotopy groups, Eilenberg--MacLane spectra represent ordinary cohomology, and Thom spectra represent bordism theories. By Brown representability, every reasonable generalized cohomology theory is represented by a spectrum.
...
\term{Stable Homotopy}
The theory of homotopy in the limit under suspension. The Freudenthal suspension theorem states that the suspension homomorphism $\pi_k(X) \to \pi_{k+1}(\Sigma X)$ is an isomorphism in a stable range of dimensions, so stable invariants are eventually constant. The stable homotopy category of spectra is the natural home for these phenomena, and stable (co)homology theories are exactly the spectrum representable ones. The stable homotopy groups of spheres $\pi_k^s$ are central objects, studied via the Adams spectral sequence.
...
The discipline concerned with drawing conclusions about a population from sample data. Point estimation seeks estimators that are unbiased and efficient, with the Cramér--Rao bound giving the best achievable variance and maximum likelihood estimators a leading method. Hypothesis testing is governed by the Neyman--Pearson lemma and likelihood ratio tests, and interval estimation produces confidence intervals. Bayesian inference treats parameters as random variables and updates prior beliefs via Bayes' theorem. It provides the mathematical foundations of modern statistics.
...
The calculus of integration for stochastic processes, generalizing the Itô integral to integration against general semimartingales. Its core tools are the Itô formula, martingales, quadratic variation, stochastic differential equations, and the Girsanov transformation for changing the probability measure. It gives rigorous meaning to noise-driven dynamics and underpins mathematical finance, where asset prices are modeled by geometric Brownian motion, and the general theory of diffusion processes.

\term{Stochastic Differential Equation}
An equation of the form $dX_t = b(X_t)\,dt + \sigma(X_t)\,dW_t$, where $W_t$ is Brownian motion and the integrals are interpreted in the Itô or Stratonovich sense. Under Lipschitz and linear-growth conditions on the coefficients, solutions exist and are pathwise unique, yielding a Markov diffusion with infinitesimal generator $b\cdot\nabla + \frac{1}{2}\operatorname{tr}(\sigma\sigma^T D^2)$. SDEs connect to PDEs through the Feynman--Kac formula, to martingale problems through their martingale characterization, and model dynamics in finance and physics.
...
\term{Stopping Time}
A random variable $T$ taking values in the time set of a filtration $(\mathcal{F}_t)$ such that the event $\{T \le t\}$ lies in $\mathcal{F}_t$ for every $t$, expressing that the decision to stop uses only information available up to time $t$, without looking into the future. Stopping times are the admissible random times in the optional stopping theorem for martingales and the strong Markov property. Hitting times, the first times a process enters a set, are the canonical examples.
...
\term{Suspension}
The operation $\Sigma X = X \times I/(X \times \{0,1\})$ that collapses both ends of the cylinder of a space $X$ to points; in the pointed setting, the reduced suspension $X \wedge S^1$ is used. Suspension is left adjoint to the loop space functor, giving the adjunction $[\Sigma X, Y] \cong [X, \Omega Y]$, and it induces the suspension homomorphism $\pi_n(X) \to \pi_{n+1}(\Sigma X)$ in homotopy groups. Iterated suspensions stabilize by the Freudenthal suspension theorem, linking to stable homotopy.
...
A counting technique: the size of the union of sets is the sum of their sizes minus the sum of sizes of pairwise intersections plus the sum of triple intersections, and so on. For sets $A_1, \ldots, A_n$: $\left|\bigcup_i A_i\right| = \sum |A_i| - \sum |A_i \cap A_j| + \sum |A_i \cap A_j \cap A_k| - \cdots$. It is used to count derangements, to sieve primes, and in probability.
\term{Simplex Method}
An algorithm for solving linear programming problems, due to George Dantzig (1947). It traverses the vertices of the feasible polytope, improving the objective at each step, until an optimum is reached. Though exponential in the worst case, it is extremely efficient in practice. The dual simplex method and interior-point methods are important variants.
\term{Sieve}
A technique in number theory for estimating the size of sifted sets of integers. The sieve of Eratosthenes is the prototype. Modern sieves include the Brun sieve, the Selberg sieve, and the large sieve, which bounds sums of the form $\sum_{n} \left|\sum_{a \bmod q} a_n\right|^2$. Sieves are fundamental to problems in additive number theory, including the twin prime conjecture and Goldbach's conjecture.
\term{Sobolev Theorem}
Also called the Sobolev embedding theorem: it states that Sobolev spaces $W^{k,p}(\Omega)$ embed continuously (or compactly) into other function spaces such as $L^q$ or $C^m$, depending on the relationship between $k$, $p$, and the dimension $n$. For $kp > n$, functions in $W^{k,p}$ are Hölder continuous. The theorem is essential for the regularity theory of elliptic PDEs.
